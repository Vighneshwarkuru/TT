...
export default function HackathonManagerPanel() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to save hackathon changes.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ animation: 'fadeIn 0.4s ease-out' }}>
      <header style={{ marginBottom: '3rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h2 style={{ fontSize: '1.75rem', fontWeight: 800, letterSpacing: '-0.03em', marginBottom: '0.5rem' }}>Manage Hackathons</h2>
              <p style={{ color: 'var(--text-muted)', fontSize: '0.9375rem', maxWidth: '600px' }}>Create and configure hackathons for evaluation.</p>
            </div>
          </div>
      </header>
      ...
      <div className="card" style={{ padding: '2.5rem', marginBottom: '4rem' }}>
        <header style={{ marginBottom: '2.5rem', borderBottom: '1px solid var(--bg-soft)', paddingBottom: '1.25rem' }}>
            <h3 style={{ fontSize: '1.125rem', fontWeight: 800 }}>{editId ? 'Edit Hackathon' : 'Create New Hackathon'}</h3>
            <p style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', marginTop: '0.25rem' }}>Set the registration and evaluation dates.</p>
        </header>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '2rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
            <div style={{ gridColumn: 'span 2' }}>
                <label style={{ display: 'block', fontSize: '0.625rem', fontWeight: 800, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '0.75rem', letterSpacing: '0.05em' }}>Hackathon Name</label>
                <input className="input" name="name" value={form.name} onChange={handleChange} required placeholder="Global Innovation Summit 2026" />
            </div>
            <div style={{ gridColumn: 'span 2' }}>
                <label style={{ display: 'block', fontSize: '0.625rem', fontWeight: 800, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '0.75rem', letterSpacing: '0.05em' }}>Description</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2rem' }}>
            <div>
              <label style={{ display: 'block', fontSize: '0.625rem', fontWeight: 800, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '0.75rem', letterSpacing: '0.05em' }}>Start Date</label>
              <input className="input" type="date" name="startDate" value={form.startDate} onChange={handleChange} required />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.625rem', fontWeight: 800, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '0.75rem', letterSpacing: '0.05em' }}>End Date</label>
              <input className="input" type="date" name="endDate" value={form.endDate} onChange={handleChange} required />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.625rem', fontWeight: 800, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '0.75rem', letterSpacing: '0.05em' }}>Registration Deadline</label>
          ...
          <div style={{ padding: '2.5rem', background: 'var(--bg-soft)', borderRadius: '16px' }}>
            <h4 style={{ fontSize: '0.75rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--accent)', marginBottom: '2rem' }}>Custom Questions</h4>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2rem' }}>
                <div>
                    <label style={{ display: 'block', fontSize: '0.625rem', fontWeight: 800, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '0.75rem', letterSpacing: '0.05em' }}>Question 01</label>
                ...
                <div>
                    <label style={{ display: 'block', fontSize: '0.625rem', fontWeight: 800, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '0.75rem', letterSpacing: '0.05em' }}>Question 02</label>
                ...
                <div>
                    <label style={{ display: 'block', fontSize: '0.625rem', fontWeight: 800, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '0.75rem', letterSpacing: '0.05em' }}>Question 03</label>
           ...
           <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginTop: '0.5rem' }}>
            ...
            <label htmlFor="isActive" style={{ fontSize: '0.9375rem', fontWeight: 800, cursor: 'pointer', color: 'var(--text-main)' }}>Set Hackathon to Active</label>
          </div>
          ...
          <div style={{ display: 'flex', gap: '1.25rem', marginTop: '1.5rem', borderTop: '1px solid var(--bg-soft)', paddingTop: '2.5rem' }}>
            <button className="btn btn-primary" type="submit" disabled={loading} style={{ padding: '1rem 3rem', fontSize: '0.875rem', fontWeight: 800, letterSpacing: '0.05em' }}>
              {loading ? 'SAVING...' : editId ? 'SAVE CHANGES' : 'CREATE HACKATHON'}
            </button>
            {editId && (
              <button 
                className="btn" 
      ...
      <header style={{ marginBottom: '2rem' }}>
          <h3 style={{ fontSize: '1.125rem', fontWeight: 800, letterSpacing: '-0.02em' }}>All Hackathons</h3>
      </header>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: 'var(--bg-soft)', borderBottom: '1px solid var(--border)' }}>
              <th style={{ padding: '1.25rem 2rem', textAlign: 'left', fontSize: '0.625rem', fontWeight: 900, textTransform: 'uppercase', color: 'var(--text-muted)', letterSpacing: '0.05em' }}>Hackathon Name</th>
              <th style={{ padding: '1.25rem 2rem', textAlign: 'left', fontSize: '0.625rem', fontWeight: 900, textTransform: 'uppercase', color: 'var(--text-muted)', letterSpacing: '0.05em' }}>Dates</th>
          ...
          <tbody>
            {hackathons.length === 0 ? (
                <tr>
                    <td colSpan="4" style={{ padding: '4rem', textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.875rem', fontWeight: 500 }}>No hackathons found.</td>
                </tr>
            ) : hackathons.map(h => (
              <tr key={h.id} style={{ borderBottom: '1px solid var(--border)', transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-soft)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                ...
                <td style={{ padding: '1.5rem 2rem', textAlign: 'right' }}>
                  <div style={{ display: 'flex', gap: '1.25rem', justifyContent: 'flex-end' }}>
                    <button onClick={() => handleEdit(h)} style={{ background: 'none', border: 'none', color: 'var(--accent)', fontSize: '0.6875rem', fontWeight: 800, cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Edit</button>
                    <button onClick={() => handleDelete(h.id)} style={{ background: 'none', border: 'none', color: 'var(--error)', fontSize: '0.6875rem', fontWeight: 800, cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Delete</button>
                  </div>
                </td>
              </tr>
            ))}
